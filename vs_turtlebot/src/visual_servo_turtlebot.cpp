...
private:
  ...
  ros::Publisher  talker_info; // Publish Talker info to pick and place task
  ros::Subscriber subPose_;  // pose_stamped
  ros::Subscriber subStatus_;  // pose_stamped
  ros::Subscriber sub_cam_info; // Camera parameters
  ros::Subscriber chatter_info; // Subscribe Chatter info from mapping task
  ...
  bool chatter; //Is equal to one if we received the message from topic that we subscribe from mapping task
  double depth;
  double Z, Zd;
  double lambda;
...
VS::VS(int argc, char**argv)
{
  ...
  // Publish message to tell pick and place task that our task is finished.
  pubTalker_ = nh_.advertise<std_msgs::String>("/vs_status", 1000);
  ...
  try {
    ...
    if ((Z <= depth * high_ratio) && (Z >= depth * low_ratio)){//IF1
        while(1){ //Infinite Loop -- Here we have finished our task
            ...
            talker_info.publish("vs_done");
            valid_pose_prev == true; //Flag that the pose already fix
            exit(1);
        }
    }//END IF1
